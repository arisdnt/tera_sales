...
export function EditProdukModal({ produk, onClose, onSave }: Props) {
    const [namaProduk, setNamaProduk] = useState(produk.nama_produk);
    const [hargaSatuan, setHargaSatuan] = useState(String(produk.harga_satuan));
    const [statusProduk, setStatusProduk] = useState(produk.status_produk);
    const [isPriority, setIsPriority] = useState(produk.is_priority ?? false);
    const [priorityOrder, setPriorityOrder] = useState(produk.priority_order ? String(produk.priority_order) : "");
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            await updateWithSync("produk", "id_produk", produk.id_produk, {
            ...
            console.error("Failed to update produk:", err);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/20">
            <div className="w-full max-w-lg bg-white shadow-2xl border border-slate-200 max-h-[90vh] flex flex-col">
                ...
                <div className="flex items-center border-b border-teal-700 bg-[#005461] px-4 h-10">
                    ...
                    <h2 className="text-sm font-bold text-white">Edit Produk #{produk.id_produk}</h2>
                </div>
